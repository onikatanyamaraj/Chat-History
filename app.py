...
with st.sidebar:
    ...
    st.selectbox("选择模型：", index=0, options=['模型1', '模型2'], key="select_model")
    st.write("\n")
    st.caption("""
    - 双击页面可直接定位输入栏
    - Ctrl + Enter 可快捷提交问题
...
tap_input, tab_func = st.tabs(['💬 聊天', '🛠️ 功能'])
...
# def get_model_input():
    ...
    # model_input = [{"uuid": uuid_input, "question": user_input}]
    ...
    #     "frequency_penalty": st.session_state["frequency_penalty" + current_chat],
    # }
    # return model_input  #, paras

# st.session_state是用于存储应用程序状态的字典，存储用户输入与聊天机器人的响应
...
if st.session_state['user_input_content'] != '':
    ...
    history_need_input = st.session_state.get("pre_user_input_content", "")
    uuid_input = str(uuid.uuid4())

    # 返回输入的历史聊天记录和模型参数，列表返回
    with st.spinner("🤔"):
        try:
            # Send a POST request to the specified URL with the model input data
            response = requests.post('http://127.0.0.1:5000/SendMessageService', json={'uuid':uuid_input,'question': history_need_input})
            if response.status_code != 200:
                print('请求失败，状态码为：', response.status_code)
                print('错误信息为：', response.text)
            # Do something with the response, such as displaying it to the user
            st.write(response.text.encode('utf-8').decode('unicode_escape'))
